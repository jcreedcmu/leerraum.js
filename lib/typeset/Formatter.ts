...
import { linebreak, box, glue, penalty, infinity } from './linebreak';
    ...
    words.forEach(function(word, index, array) {
      ...
      if (hyphenated.length > 1 && word.length > 4) {
        hyphenated.forEach(function(part, partIndex, partArray) {
          nodes.push({ style: span, value: box(measureText(span.fontFamily, span.fontSize, part), part) });

          if (partIndex !== partArray.length - 1) {
            nodes.push({ style: span, value: penalty(hyphenWidth, hyphenPenalty, 1) });
          }
        });
      }
      else {
        nodes.push({ style: span, value: box(measureText(span.fontFamily, span.fontSize, word), word) });
      }

      if (spanIndex === spanArray.length - 1 && index === array.length - 1) {
        nodes.push({ style: span, value: glue(0, infinity, 0) });
        nodes.push({ style: span, value: penalty(0, -infinity, 1) });
      }
      // don't add space after a span
      else if (index !== array.length - 1) {
        nodes.push({ style: span, value: glue(spaceWidth, spaceStretch, spaceShrink) });
      }
    });
  });
  ...
  spans.forEach(function(span, spanIndex, spanArray) {
    ...
    words.forEach(function(word, index, array) {
      ...
      // don't add space after a span
        ...
        nodes.push({ style: span, value: glue(0, 12, 0) });
        nodes.push({ style: span, value: penalty(0, 0, 0) });
        nodes.push({ style: span, value: glue(spaceWidth, -12, 0) });
      }
    });
  });
...
export function center(measureText: (fontFamily: string, fontSize: number, text: string) => number, hypher, spans: T.Span[], options): T.Node[] {
  ...
  if (spans.length > 0) {
    nodes.push({ style: spans[0], value: box(0, '') });
    nodes.push({ style: spans[0], value: glue(0, 12, 0) });
  }
  ...
  spans.forEach(function(span, spanIndex, spanArray) {
    ...
    words.forEach(function(word, index, array) {
      ...
      if (spanIndex === spanArray.length - 1 && index === array.length - 1) {
        nodes.push({ style: span, value: glue(0, 12, 0) });
        nodes.push({ style: span, value: penalty(0, -infinity, 0) });
      }
      // don't add space after a span
      else if (index !== array.length - 1) {
        nodes.push({ style: span, value: glue(0, 12, 0) });
        nodes.push({ style: span, value: penalty(0, 0, 0) });
        nodes.push({ style: span, value: glue(spaceWidth, -24, 0) });
        nodes.push({ style: span, value: box(0, '') });
        nodes.push({ style: span, value: penalty(0, infinity, 0) });
        nodes.push({ style: span, value: glue(0, 12, 0) });
      }
    });
